    ...
    public partial class frmCrearUsuario : Form
    {
        ...
        private void btnGuardar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Por favor completa todos los campos obligatorios (nombre, contraseña, rol).");
                    return;
                }

                if (rol == "Cliente")
                {
                    Cliente cliente = new Cliente(nombre, apellido, contrasena, rol, correo, telefono, direccion);
                    cliente.GuardarEnBD(conexionBD);
                }
                else if (rol == "Veterinario")
                {
                    Veterinario vet = new Veterinario(nombre, apellido, contrasena, rol, especialidad);
                    vet.GuardarEnBD(conexionBD);
                }
                else if (rol == "Administrador")
                {
                    Administrador admin = new Administrador(nombre, apellido, contrasena, rol);
